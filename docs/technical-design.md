...
- Деплой на GitHub Pages: `vite.config.ts` берёт `base` из env `BASE_PATH` (локально/dev — `/`, в workflow деплоя задаётся `/gamo-craft`), `.github/workflows/deploy-pages.yml` (npm `ci` -> `icons` -> `build` с `BASE_PATH` -> копия `dist/index.html` в `dist/404.html` для SPA-маршрутов BrowserRouter -> `upload-pages-artifact` -> `deploy-pages`); источник Pages должен быть «GitHub Actions» (не «Deploy from branch», который публикует сырой репозиторий без сборки). `BrowserRouter` в `src/main.tsx` получает `basename` из `import.meta.env.BASE_URL` (без хвостового слэша) — без него в подкаталоге `/gamо-craft/` роутер уходил в fallback-роут и редиректил на корень сайта. Все runtime-ссылки на ассеты (текстуры resource-pack, постеры, шрифт) — относительные от документа (без ведущего `/`), чтобы одинаково работать на root, в подкаталоге и на кастомном домене; шрифт пакета загружается через `FontFace` API в `applyResourcePack` (а не `@font-face` во внешнем CSS, где `url()` резолвится относительно файла стилей).
