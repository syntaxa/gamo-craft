...
Для текущего 5-task UI `Математика - серебряный` использует повышенную награду: `correctAnswer = 20`, `lessonAccuracy80 = 50`, максимум `150` котокоинов. Операции: `mul` (`a,b` в `2..9`, `a*b <= 20`) и `div` (частное и делитель в `2..9`, делимое `= частное * делитель`, всегда целое).
...
- Layout: `.app-shell` использует `height: 100dvh` (fallback `100vh`) и grid `grid-template-rows: auto 1fr`; шапка/бренд отсутствуют, индикатор валюты вынесен в одну строку с навигацией (`.main-nav-row`, `.currency-badge` с `z-index` и `overflow`-изоляцией от прокрутки пунктов меню); `.screen-wrap` прокручивается внутри; в Build-режиме `.build-screen` (flex:1) + `.build-stage` (flex:1 1 auto, `min-height: 0`, `touch-action: none`) занимают всю свободную высоту под панелью навигации.
- На coarse-pointer устройствах (`@media (pointer: coarse)`): скрываются `.build-controls-hint`, `.build-hud` (карточка-подпись) и desktop-подсказки; hotbar поднимается на `bottom: 168px`, чтобы не перекрываться кнопками; компактный чип `.build-touch-hint` удален и не отображается; подсказка паузы наложена по центру окна мира (`.build-pause-hint`), не съедая высоту сцены.
...
- Full inventory закрывается клавишей `E`/`Esc`, повторной touch-кнопкой и тапом/кликом по фону за пределами модального окна (`inventory-overlay` onPointerDown с проверкой `target === currentTarget`).
...
- Генерация задач `Математика - серебряный`: только умножение и деление, диапазон до `20` (множители `2..9`, произведение/делимое `<= 20`, деление без остатка), чередование операций и повышенная награда до `150` котокоинов в текущем 5-task flow.
...
- Экран яиц: `egg_meme` стоит `200` котокоинов и выдает poster item категории `cats`; `egg_sbear` стоит `400` котокоинов и выдает poster item категории `sbearadventure`. Если награда не помещается в инвентарь, котокоины возвращаются и показывается сообщение об ошибке (награда не теряется молча).
...
- Деплой на GitHub Pages: `vite.config.ts` берёт `base` из env `BASE_PATH` (локально/dev — `/`, в workflow деплоя задаётся `/gamo-craft`), `.github/workflows/deploy-pages.yml` (npm `ci` -> `icons` -> `build` с `BASE_PATH` -> копия `dist/index.html` в `dist/404.html` для SPA-маршрутов BrowserRouter -> `upload-pages-artifact` -> `deploy-pages`); источник Pages должен быть «GitHub Actions» (не «Deploy from branch», который публикует сырой репозиторий без сборки). `BrowserRouter` в `src/main.tsx` получает `basename` из `import.meta.env.BASE_URL` (без хвостового слэша) — без него в подкаталоге `/gamо-craft/` роутер уходил в fallback-роут и редиректил на корень сайта. Все runtime-ссылки на ассеты resource-pack строятся base-aware хелпером `packAsset()` в `src/theme/resourcePacks.ts` (`import.meta.env.BASE_URL` + относительный путь внутри пакета): абсолютные URL нужны, т.к. `url()` внутри CSS-переменных резолвится по месту использования во внешнем bundled stylesheet (в `assets/`), а не относительно документа; шрифт пакета загружается через `FontFace` API в `applyResourcePack`. Постеры (`assets/posters/...`) остаются документ-относительными — они идут только в `img src`.
